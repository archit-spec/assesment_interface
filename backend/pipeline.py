...
class ETLPipeline:
    ...
    def calculate_summary(self, data: List[Dict[str, Any]], file_type: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if amount_col in df.columns:
                ...
                if not pd.api.types.is_numeric_dtype(df[amount_col]):
                    ...
                    cleaned_values = df[amount_col].astype(str).str.replace('$', '').str.replace(',', '')
                ...
                logger.error(f"Amount column {amount_col} not found in columns: {df.columns.tolist()}")
            ...
            if type_col in df.columns:
                ...
                type_stats = df.groupby(type_col)[amount_col].agg(['count', 'sum']).round(2)
                ...
                logger.error(f"Type column {type_col} not found in columns: {df.columns.tolist()}")
    ...
    async def load(self, unprocessed_id: int, processed_data: List[Dict[str, Any]], 
                  summary: Dict[str, Any], file_type: str) -> ProcessedData:
        ...
        try:
            ...
            unprocessed = self.db.query(UnprocessedData).filter(
                UnprocessedData.id == unprocessed_id
            ).first()
            if unprocessed:
    ...
    async def process_file(self, unprocessed_id: int) -> ProcessedData:
        ...
        try:
            ...
            if not unprocessed:
                raise ValueError(f"No unprocessed record found with id {unprocessed_id}")
