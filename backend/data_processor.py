...
def process_mtr_file(file_path: str) -> pd.DataFrame:
    ...
    try:
            ...
            if file_ext == ".csv":
                ...
                for engine in excel_engines:
                    try:
                        ...
                        logger.info(
                            f"Successfully read Excel file using {engine} engine"
                        )
                        break
        ...
        logger.info(
            f"Transaction Types: {mtr_df['Transaction Type'].unique().tolist()}"
        )
...
def process_payment_file(file_path: str) -> pd.DataFrame:
    ...
    try:
        ...
        try:
            df = pd.read_csv(
                file_path, quoting=1
            )  # QUOTE_ALL to handle newlines in cells
        ...
        payment_df["Net Amount"] = (
            payment_df["Net Amount"].str.replace(",", "").astype(float)
        )
