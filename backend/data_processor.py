...
def process_mtr(file_content: bytes) -> pd.DataFrame:
    ...
    try:
        ...
        # Apply replacements
        mtr_df.loc[:, 'Transaction Type'] = mtr_df['Transaction Type'].replace(type_mapping)
...
def process_payment(file_content: bytes) -> pd.DataFrame:
    ...
    try:
        ...
        # Apply replacements
        payment_df.loc[:, 'Payment Type'] = payment_df['Payment Type'].replace(value_mapping)
...
def process_merged_data(merged_df: pd.DataFrame) -> Dict[str, pd.DataFrame]:
    ...
    try:
        ...
        results['removal_orders'] = valid_orders[valid_orders['Order Id'].str.len() == 10].copy()
        ...
        logger.info(f"Found {len(results['order_payment_received'])} orders with payment received")
        
        results['order_not_applicable'] = valid_orders[
            (valid_orders['Net Amount'].notna()) & 
            (valid_orders['Invoice Amount'].isna())
        ].copy()
        logger.info(f"Found {len(results['order_not_applicable'])} not applicable orders")
...
def calculate_tolerance(df: pd.DataFrame) -> pd.DataFrame:
    ...
    try:
        ...
        logger.info(f"Tolerance calculation complete. Results: {tolerance_counts.to_dict()}")
    ...
    try:
        ...
        response_data = {
            ...
            'categories': {
                category: {
                    'count': len(data),
                    'invoice_amount': float(data['Invoice Amount'].sum()) if 'Invoice Amount' in data else 0,
                    'net_amount': float(data['Net Amount'].sum()) if 'Net Amount' in data else 0
                }
                for category, data in categorized_data.items()
            },
            'tolerance': {
                status: int(count)
                for status, count in tolerance_checked['Tolerance_Status'].value_counts().items()
                if pd.notna(status)
            }
        }
