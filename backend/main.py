...
async def upload_mtr(file: UploadFile = File(...), db: Session = Depends(get_db)):
    ...
    try:
        ...
        file_path = (
            UPLOAD_DIR
            / f"mtr_{datetime.now().strftime('%Y%m%d_%H%M%S')}_{file.filename}"
        )
...
async def upload_payment(file: UploadFile = File(...), db: Session = Depends(get_db)):
    ...
    try:
        ...
        file_path = (
            UPLOAD_DIR
            / f"payment_{datetime.now().strftime('%Y%m%d_%H%M%S')}_{file.filename}"
        )
        logger.info(f"Saving file to: {file_path}")
...
async def get_transactions(
    ...
    try:
        ...
        records = (
            ...
            .order_by(desc(ProcessedData.processing_timestamp))
            .all()
        )
        ...
        all_transactions.sort(
            key=lambda x: datetime.strptime(x.get("date/time", "1/1/1900"), "%m/%d/%Y"),
            reverse=True,
        )
...
async def upload_file(report_type: str, file: UploadFile = File(...)):
    ...
    try:
        ...
        if report_type == "payment" and not file.filename.endswith(".csv"):
            raise HTTPException(
                status_code=400, detail="Payment report must be a CSV file"
            )

        if report_type == "mtr" and not file.filename.endswith(".xlsx"):
            raise HTTPException(
                status_code=400, detail="MTR report must be an Excel file"
            )
        ...
        # Log file upload
        logger.info(
            f"Received {report_type} report: {file.filename} ({len(contents)} bytes)"
        )
